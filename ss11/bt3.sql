...
-- 2
delimiter //
create procedure GetCustomerByPhone (in in_PhoneNumber varchar(10))
begin 
   select CustomerID, FullName, DateOfBirth, Address, Email from Customers 
   where in_PhoneNumber = PhoneNumber ;
end //
delimiter ;

-- 3
delimiter //
create procedure GetTotalBalance (in in_CustomerID int , out TotalBalance decimal(15,2) )
begin 
  select sum(Balance) into TotalBalance 
  from Accounts 
  where in_CustomerID = CustomerID ;
end //
delimiter ;

SET @balance = 0;
CALL GetTotalBalance(1, @balance);
SELECT @balance AS TotalBalance;

-- 4
delimiter //
create procedure IncreaseEmployeeSalary (in in_EmployeeID int ,  inout salary_now decimal(10,2))
begin 
  update Employees
  set salary = salary *1.10 
  where in_EmployeeID = EmployeeID ;
  
  select salary into salary_now
  from Employees 
  where in_EmployeeID = EmployeeID ;
end //
delimiter ;
...
set @total = 0 ;
call IncreaseEmployeeSalary (1,@total) ;
