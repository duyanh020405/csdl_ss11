...
-- 2
DELIMITER //
create Procedure UpdateSalaryByID (in in_EmployeeID int ,out salary_new decimal(10,2))
begin
select salary into salary_new 
from Employees
where EmployeeID = in_EmployeeID ;

    if salary_new > 20000000 then 
    set salary_new = salary_new*1.5 ;
    else set salary_new = salary_new * 1.1; 
    end if;
    ...
    select * from Employees where EmployeeID = in_EmployeeID;
end //
delimiter ;
SET SQL_SAFE_UPDATES = 0;
set @total = 0;
call UpdateSalaryByID (2,@total);	 

-- 3
DELIMITER //
create Procedure GetLoanAmountByCustomerID (in in_CustomerID  int ,out money_pay double)
begin
 declare rs int ;
   set rs = (select count(customerid) from loans where customerid = in_customerid);
   if rs > 0 
    then  select LoanAmount into money_pay from loans where customerid = in_customerid limit 1 ;
   else 
   set money_pay = 0 ;end if;
	
	-- select LoanAmount into money_pay from loans where customerid = in_customerid;
--     if money_pay is null or 0 then set money_pay = 0 ;end if ;
end //
DELIMITER ;
drop procedure GetLoanAmountByCustomerID ;

set @money_pay = 0;
call GetLoanAmountByCustomerID (2,@money_pay);
select @money_pay ;
